 ...
 <input type="button" name="submitbtn" id="btn" class="btn btn-info" value="Multiple Update" />                   
             ...
             <div>
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php
                      ...
                      $get_demande="SELECT * FROM users, demande  WHERE users.user_id=demande.user_id_vend and  demande.etat_dmd='no ramassé' and users.user_id=demande.user_id_ram";
                      ...
                      $nameR=$rows['user_name'];
                      $ramasseur_id =$rows['user_id'];
   ...
   <select  name="ramasseur" id="s" width="144" style="height:19px; width:150px; padding-top:-10px;">
        <option  selected disabled value="<?php echo $ramasseur_id; ?>"> <?php echo $nameR; ?></option>
                 <?php 
                              
                             $getRs = "select * from users where users.user_role='Ramasseurs'";
                              $runRs = mysqli_query($con,$getRs);
                              
                              while ($rowR=mysqli_fetch_array($runRs)){
                                  
                                  $ramasseurID = $rowR['user_id'];
                                  $nameRS = $rowR['user_name'];
                                  
                                  echo "
                                  
                                  <option class='row_value' value='$ramasseurID'> $nameRS </option>
       ...
       if (is_array($values) || is_object($values))
{
    ...
    $value->idr;
        ...
        $query = " UPDATE demande SET ramasseur_id = $value->idr , etat_dmd='no ramassé' WHERE dmd_id =  $value->idd ";
    ...
    var ids = [];
     ...
     var tbl = document.getElementById('example1');
        ...
        for (var i = 1; i < tbl.rows.length-1 ;i++) {
            var ramasid1=tbl.rows[i].cells[8].getElementsByTagName("select")[0].selectedIndex;
